...
exports.getGalleryCategories = exports.deleteGalleryPost = exports.updateGalleryPost = exports.createGalleryPost = exports.getGalleryDetail = exports.getGalleryList = void 0;
 ...
 * 포토갤러리 목록 조회 (그리드 형태)
 */
const getGalleryList = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const page = parseInt(req.query.page) || 1;
        const limit = parseInt(req.query.limit) || 12; // 갤러리는 더 많이 표시
        ...
        let queryParams = ['gallery', status];
        if (category) {
 ...
 * 포토갤러리 상세 조회
 */
const getGalleryDetail = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        // 조회수 증가
        yield database_1.default.execute('UPDATE board_posts SET view_count = view_count + 1 WHERE id = ? AND board_type = ?', [id, 'gallery']);
        // 갤러리 상세 정보 조회
    ...
    `, [id, 'gallery']);
        ...
        const post = posts[0];
        // 첨부 이미지들 조회
        const [images] = yield database_1.default.execute(`
      SELECT 
        id,
        original_name,
        stored_name,
 ...
 * 포토갤러리 작성
 */
const createGalleryPost = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // multer로 처리된 파일들과 폼 데이터
        const files = req.files;
        const { title, content_html, content_json, content_text, excerpt, author_name, author_email, author_phone, password, category_id, featured_image } = req.body;
        // 필수 필드 검증
        ...
        // uploads 디렉토리 확인/생성
        const uploadsDir = path_1.default.join(process.cwd(), 'uploads/gallery');
    ...
    `, [
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            const filePath = path_1.default.join(uploadsDir, storedName);
...
exports.createGalleryPost = createGalleryPost;
/**
 * 포토갤러리 수정
 */
const updateGalleryPost = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const { title, content_html, content_json, content_text, excerpt, password, category_id, featured_image, deletedImageIds } = req.body;
        const files = req.files;
        // 기존 게시물 조회
        const [posts] = yield database_1.default.execute('SELECT password FROM board_posts WHERE id = ? AND board_type = ?', [id, 'gallery']);
        if (posts.length === 0) {
            return res.status(404).json({
                success: false,
                message: '갤러리를 찾을 수 없습니다.'
        ...
        if (deletedImageIds) {
            ...
            if (imageIdsToDelete.length > 0) {
                // 삭제할 이미지 파일 경로들 조회
                const [imagesToDelete] = yield database_1.default.execute(`
        ...
        `, [...imageIdsToDelete, id]);
                ...
                for (const imageInfo of imagesToDelete) {
                    const fullPath = path_1.default.join(__dirname, '..', imageInfo.file_path);
        ...
        const [remainingImages] = yield database_1.default.execute(`
      SELECT file_path FROM board_attachments 
      WHERE post_id = ? 
      ORDER BY id ASC 
      LIMIT 1
    `, [id]);
        ...
        if (remainingImages.length > 0) {
            newFeaturedImage = remainingImages[0].file_path;
        }
        else {
...
exports.updateGalleryPost = updateGalleryPost;
/**
 * 포토갤러리 삭제
 */
const deleteGalleryPost = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
...
exports.deleteGalleryPost = deleteGalleryPost;
/**
 * 포토갤러리 카테고리 목록 조회
 */
